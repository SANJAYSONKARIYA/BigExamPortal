document.addEventListener('DOMContentLoaded', function () {
    const states = {
        'Andaman and Nicobar' : ["Nicobar","North Middle Andaman","South Andaman"],
        ...
        'Ladakh' : ["Kargil","Leh","Changtang","Dras","Nubra","Sham Valley","Zanskar"],
    ...
    if (!stateSelect || !districtSelect) {
        console.error('Dropdown elements not found. Ensure the HTML contains elements with IDs "state" and "district".');
        return;
    }
    ...
    // Update districts dropdown when state is changed
    stateSelect.addEventListener('change', function () {
        const selectedState = this.value;
        const districts = states[selectedState] || [];
    ...
    // Trigger change event to populate districts for the default selected state
...
function validateFile(input) {
    ...
    const minSize = 20 * 1024; // 20KB in bytes
    ...
    if (file) {
        ...
        if (file.size < minSize) {
            alert('File size is too small. Minimum size is 20KB.');
            input.value = ''; // Clear the input
            return;
        }
